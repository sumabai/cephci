...
def run(ceph_cluster, **kw):
    ...
    try:
        ...
        cephfs_config = {}
        run_time = config.get("run_time", 300)
...
def io_test_runner(
    ...
    log.info(f"Running {io_test} on {sv_info}")
    ...
    if io_test == "io_test_workflow_7" or io_test == "io_test_workflow_8":
...
def io_test_workflow_1(run_time, log_path, clients, fs_system_utils, sv_info):
    ...
    log1.info(f"Test Object : {sv_info}")
    for i in sv_info:
        sv_name = i
    ...
    file_name = f"{dir_path}/io_test_workflow_1"
    file_ops = {
        "write": f"fio --name={file_name} --ioengine=libaio --size 1Gb --rw=write --direct=0",
        "read": f"fio --name={file_name} --ioengine=libaio --size 1Gb --rw=read --direct=0 --startdelay=1",
    }
...
def io_test_workflow_2(run_time, log_path, clients, fs_system_utils, sv_info):
    ...
    while datetime.datetime.now() < end_time:
        ...
        with parallel() as p:
            for io_type in ["overwrite", "read"]:
                cmd = f"{smallfile_cmd} --operation {io_type} --threads 1 --file-size 10240 --files 1 --top {dir_path}"
                cmd += f" --network-sync-dir /var/tmp/smallfile_dir_{rand_str}"

                p.spawn(run_cmd, cmd, client, log1)
        client.exec_command(sudo=True, cmd=f"rm -rf /var/tmp/smallfile_dir_{rand_str}")
...
def io_test_workflow_3(run_time, log_path, clients, fs_system_utils, sv_info):
...
def io_test_workflow_4(run_time, log_path, clients, fs_system_utils, sv_info):
    ...
    while datetime.datetime.now() < end_time:
        ...
        with parallel() as p:
            for i in range(0, 5):
                log1.info(f"Running cmd Iteration {i}")
                p.spawn(run_cmd, cmd, client, log1)
...
def io_test_workflow_5(run_time, log_path, clients, fs_system_utils, sv_info):
    ...
    while datetime.datetime.now() < end_time:
        cmd = f"{smallfile_cmd} --operation overwrite --threads 1 --file-size 1024000 --files 1 --top {dir_path}"
        log1.info(f"Executing cmd {cmd}")
        client.exec_command(
            sudo=True,
            cmd=cmd,
...
def io_test_workflow_6(run_time, log_path, clients, fs_system_utils, sv_info):
...
def io_test_workflow_8(run_time, log_path, clients, fs_system_utils, sv_info_list):
    ...
    while datetime.datetime.now() < end_time:
        for sv_name in sv_info_objs:
            ...
            sv_info_objs[sv_name].update(
                {"file_ops": file_ops, "file_ops_1": file_ops_1}
            )
        ...
        with parallel() as p:
            for sv_name in sv_info_objs:
                ...
                file_ops_1 = sv_info_objs[sv_name]["file_ops_1"]
                for op_type in random.choice([file_ops, file_ops_1]):
...
def io_test_workflow_10(run_time, log_path, clients, fs_system_utils, sv_info):
